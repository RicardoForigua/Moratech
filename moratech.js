...
// Carrusel infinito de proveedores (logos)
(function() {
  ...
  let pos = 0;
  let paused = false;
  const speed = 0.9; // píxeles por frame aprox (ajusta si lo quieres más rápido)

  function step() {
    if (!paused) {
      pos -= speed;
      const maxScroll = track.scrollWidth / 2; // porque duplicamos el contenido

      // cuando ya se desplazó el "largo real", reinicia sin corte
...
// === ScrollSpy automático para navbar ===
(function() {
  ...
  const observer = new IntersectionObserver(
    ...
    {
      root: null,
      threshold: 0.45, // porcentaje visible para activar (ajustable)
    }
  );
